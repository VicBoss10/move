...
export class AirQualityCardComponent {
  ...
  // Exponemos el utility para usar en el template
  ColorUtility = ComponentColorUtility;

  private isLoading$ = new BehaviorSubject<boolean>(true);

  /**
   * Observable que emite el estado de las métricas
   * Usa getLatest() para obtener solo el registro más reciente (más eficiente)
   */
  ...
  constructor(private sensorDataService: SensorDataService) {
    this.metrics$ = this.sensorDataService.getLatest().pipe(
      ...
      catchError((err) => {
        console.error("Error cargando datos de aire:", err);
