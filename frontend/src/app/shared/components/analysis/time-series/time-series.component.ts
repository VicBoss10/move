...
const METRICS: MetricOption[] = [
  ...
  { key: 'temperature', label: 'Temperatura',  unit: '°C',      border: '#eab308', bg: 'rgba(234, 179, 8, 0.1)'   },
  { key: 'humidity',    label: 'Humedad',      unit: '%',       border: '#3b82f6', bg: 'rgba(59, 130, 246, 0.1)'  },
...
export class TimeSeriesComponent implements OnInit, OnDestroy {
  ...
  private fetchAndAggregate(filters: FilterState) {
    ...
    return combineLatest([sensor$, vehicles$]).pipe(
      catchError(err => {
      ...
      switchMap(result => {
        ...
        const metricPoints: (number | null)[] = slots.map(slot => {
          const inSlot = sensorData.filter(d =>
        ...
        const vehicleCounts: number[] = slots.map(slot =>
        ...
        const stats: TimeSeriesStats = { metricMin, metricAvg, metricMax, vehicleTotal, metricOption };
  ...
  private periodConfig(period: PeriodKey): { start: Date; end: Date; granularity: GranularityKey } {
    const end = new Date();
    switch (period) {
      case '7d':  return { start: new Date(end.getTime() - 7  * 24 * 3_600_000), end, granularity: '4h'  };
      case '30d': return { start: new Date(end.getTime() - 30 * 24 * 3_600_000), end, granularity: 'day' };
      default:    return { start: new Date(end.getTime() -      24 * 3_600_000), end, granularity: 'hour'};
    }
  }
